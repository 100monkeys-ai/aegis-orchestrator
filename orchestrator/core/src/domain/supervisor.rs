...
impl Supervisor {
    ...
    pub async fn run_loop(
        ...
        let overall_timeout_secs = runtime_config.resources.timeout_seconds
        ...
        let per_iteration_timeout = Duration::from_secs(
            overall_timeout_secs.checked_div((max_retries as u64).max(1)).unwrap_or(overall_timeout_secs)
        );
        ...
        match tokio::time::timeout(overall_timeout, self.run_loop_inner(
            runtime_config,
            input,
            max_retries,
            observer,
...
mod tests {
    ...
    async fn test_supervisor_success_first_iteration() {
        ...
        let result = supervisor.run_loop(create_test_config(), create_test_input(), 3, observer.clone(), CancellationToken::new()).await;
    ...
    async fn test_supervisor_terminates_instances() {
        ...
        let _result = supervisor.run_loop(create_test_config(), create_test_input(), 3, observer, CancellationToken::new()).await;
    ...
    async fn test_supervisor_overall_timeout() {
        ...
        let runtime = Arc::new(
            MockRuntime::new()
                ...
                .with_execute_success(vec!["Output".to_string(), "Output".to_string(), "Output".to_string()])
        ...
        let result = supervisor.run_loop(config, create_test_input(), 3, observer.clone(), CancellationToken::new()).await;
    ...
    async fn test_supervisor_cancellation() {
        ...
        let result = supervisor.run_loop(config, create_test_input(), 3, observer.clone(), token).await;
    ...
    async fn test_supervisor_uses_default_timeout_when_none() {
        ...
        let result = supervisor.run_loop(config, create_test_input(), 3, observer, CancellationToken::new()).await;
