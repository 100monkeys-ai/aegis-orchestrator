...
impl LocalStorageProvider {
    ...
    pub fn new(base_path: impl Into<PathBuf>) -> Result<Self, StorageError> {
        ...
        std::fs::remove_file(&test_file)
            .map_err(|e| StorageError::IoError(format!("Failed to cleanup test file: {}", e)))?;
...
impl StorageProvider for LocalStorageProvider {
    ...
    async fn delete_directory(&self, path: &str) -> Result<(), StorageError> {
        ...
        std::fs::remove_dir_all(&fs_path).map_err(|e| {
            StorageError::IoError(format!("Failed to delete directory {}: {}", path, e))
        })?;
    ...
    async fn get_usage(&self, path: &str) -> Result<u64, StorageError> {
        ...
        Self::calculate_size(&fs_path).map_err(|e| {
            StorageError::IoError(format!("Failed to calculate usage for {}: {}", path, e))
        })
    }
    ...
    async fn health_check(&self) -> Result<(), StorageError> {
        ...
        std::fs::write(&test_file, b"health-check").map_err(|e| {
            StorageError::IoError(format!("Health check failed (not writable): {}", e))
        })?;

        std::fs::remove_file(&test_file)
            .map_err(|e| StorageError::IoError(format!("Health check cleanup failed: {}", e)))?;
    ...
    async fn list_directories(&self, path: &str) -> Result<Vec<String>, StorageError> {
        ...
        for entry in std::fs::read_dir(&fs_path).map_err(|e| {
            StorageError::IoError(format!("Failed to list directory {}: {}", path, e))
        })? {
    ...
    async fn open_file(&self, path: &str, mode: OpenMode) -> Result<FileHandle, StorageError> {
        ...
        match mode {
            ...
            OpenMode::Create => {
                File::create(&fs_path).map_err(|e| {
                    StorageError::IoError(format!("Failed to create {}: {}", path, e))
                })?;
            }
        };
    ...
    async fn readdir(&self, path: &str) -> Result<Vec<DirEntry>, StorageError> {
        ...
        for entry in std::fs::read_dir(&fs_path).map_err(|e| {
            StorageError::IoError(format!("Failed to read directory {}: {}", path, e))
        })? {
            let entry =
    ...
    async fn create_file(&self, path: &str, _mode: u32) -> Result<FileHandle, StorageError> {
        ...
        if let Some(parent) = fs_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                StorageError::IoError(format!("Failed to create parent dirs: {}", e))
            })?;
        }
    ...
    async fn rename(&self, from: &str, to: &str) -> Result<(), StorageError> {
        ...
        std::fs::rename(&from_path, &to_path).map_err(|e| {
            StorageError::IoError(format!("Failed to rename {} to {}: {}", from, to, e))
        })?;
...
mod tests {
    ...
    async fn test_set_quota_and_get_usage() {
        ...
        // Set quota
        provider
            .set_quota("/test-volume", 1024 * 1024)
            .await
            .unwrap();
    ...
    async fn test_nested_directories() {
        ...
        provider
            .create_directory("/tenant-1/execution-123/workspace")
            .await
            .unwrap();

        // Verify it exists
        let fs_path = temp_dir
            .path()
            .join("tenant-1")
            .join("execution-123")
            .join("workspace");
        ...
        // Calculate usage
        let usage = provider
            .get_usage("/tenant-1/execution-123/workspace")
            .await
            .unwrap();
        assert!(usage < 1024);
    }
