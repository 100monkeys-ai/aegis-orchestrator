...
fn make_execution(max_iterations: u8) -> Execution {
    Execution::new(AgentId::new(), make_input("test task"), max_iterations)
}
...
fn new_execution_stores_agent_id() {
    ...
    let exec = Execution::new(agent_id, make_input("task"), 5);
...
fn execution_info_from_completed_execution() {
    let agent_id = AgentId::new();
    let mut exec = Execution::new(agent_id, make_input("task"), 5);
    exec.start();
    exec.complete();
